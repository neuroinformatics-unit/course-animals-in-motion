...
The 3.5-min dataset presented here consists of 44 trajectories of zebras (_Equus quagga_) expressed in a coordinate system fixed to the ground. Each individual has two keypoints (`head` and `tail`). The original trajectory data was extracted using [`SLEAP`](https://sleap.ai/) on a video clip recorded from a camera drone, and then expressed in a coordinate system fixed to the ground using [Structure-from-motion](https://en.wikipedia.org/wiki/Structure_from_motion) (with [`OpenSFM`](https://github.com/mapillary/OpenSfM) and [`OpenDroneMap`](https://github.com/OpenDroneMap/OpenDroneMap)). Note that this transformation was necessary to be able to disentangle the motion of the zebras from the movement of the camera drone. After this coordinate transformation, the data was cleaned by removing low-confidence keypoints and implausible data points.
...
We can see there is some variability in the body lengths per individual. Part of it may reflect the diversity across individuals, but from visual inspection of the videos we expect the majority of it to be due to imperfect tracking of the keypoints. To remove some of these outliers, we continue the analysis considering only body vectors that are within 2 standard deviations of the mean.
...
We can verify that indeed their norms are now equal to 1.
...
The norm of the herd vector already gives us an intuition of how aligned the whole herd is. When its norm is close to 1, it means that the majority of the unit body vectors are aligned. When its norm is close to 0, it means that the majority of the unit body vectors are dispersed.  The norm of the herd vector is sometimes called **polarization**.
...
The plot suggests that the herd goes through a few periods of high and low polarization.
