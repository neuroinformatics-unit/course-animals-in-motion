...
The 3.5-min dataset presented here consists of 44 trajectories of zebras (_Equus quagga_) expressed in a coordinate system fixed to the ground. Each individual has two keypoints (`head` and `tail`). 

The data was collected with a camera drone that followed the herd and recorded video data. Note that since both camera and animals are in motion, applying a pose estimation directly to the video data would confound both sources of movement. We need the trajectories of the animals relative to the ground, rather than relative to the drone. We computed the ground trajectories as follows: first, a trained [`SLEAP`](https://sleap.ai/) model was run on a video clip recorded from the drone. This way we obtained the trajectories of the zebras in a coordinate system linked to the drone. Then, we computed the position and orientation of the camera drone at each timestep by applying [Structure-from-motion](https://en.wikipedia.org/wiki/Structure_from_motion) (with [`OpenSFM`](https://github.com/mapillary/OpenSfM) and [`OpenDroneMap`](https://github.com/OpenDroneMap/OpenDroneMap)). This allowed us to express the trajectories in a coordinate system fixed to the ground, and allowed us to disentangle the motion of the zebras from the movement of the camera drone. After this coordinate transformation, the data was cleaned by removing low-confidence keypoints and implausible data points.
...
We can see there is some variability in the body lengths per individual. Part of it may reflect the diversity across individuals, but from visual inspection of the video we expect the majority of it to be due to imperfect tracking of the keypoints. To remove some of these outliers, we continue the next part of the analysis considering only body vectors that are within 2 standard deviations of the mean.
...
The norm of the herd vector will be bounded between 0 and 1, because it is the mean of a set of unit vectors. This is convenient because it already gives us an intuition of how aligned the whole herd is. When the herd vector norm is close to 1, it means that the majority of the unit body vectors are aligned. When its norm is close to 0, it means that the unit body vectors are dispersed. The norm of the herd vector is sometimes called **polarization**.
...
To visually check this matches the input data, we can plot polarization in time along with the unit body vectors per individual, the herd vector and the drone video. We did that in the animated plot below. The plot on the top right shows the unit body vectors (in black) and the herd vector (in red), whose norm is the polarization value. The purple line under the herd vector is the equivalent unit vector. 

::: {#fig-zebras-polarization}
<video src="https://github.com/neuroinformatics-unit/zebras-stitching/releases/download/zebras-clip/zebras_20250912_174038_w1440_crf23.mp4" autoplay loop muted playsinline width="100%"></video>

Polarization over time alongside the unit body vectors, the herd vector and the drone video.
:::
...
We can now plot the speed of each individual over time as an array.
...
As a final step, let's look at how the spatial arrangement of the herd changes over time. A convenient way to summarise this is the **pairwise distance matrix**: an `individuals` x `individuals` array whose element $(i, j)$ is the distance between individuals $i$ and $j$ at a given time.

We can compute it in one go by exploiting `xarray`'s broadcasting and with a renaming trick. If we rename the `individual` dimension of the `centroid` array to `individual_i` in one copy and to `individual_j` in another they won't be considered the same dimension by `xarray`. Then, if we subtract them we get an array with the vector from every individual $j$ to every individual $i$, at every timestep. The norm of those vectors is the distance matrix we are after.

```{python}
vector_j_to_i = centroid.rename({"individual": "individual_i"}) - centroid.rename(
    {"individual": "individual_j"}
)

dist_matrix = compute_norm(vector_j_to_i)
```

The resulting array has `(time, individual_i, individual_j)` dimensions. It is symmetric, and its diagonal is zero, since the distance of an individual to itself is zero.

Because the matrix changes at every frame, an animation is a natural way to inspect it. Black rows and columns that cross correspond to individuals that were not detected.

```{python}
#| code-fold: true
#| label: fig-distance-matrix-animation
#| fig-cap: "Pairwise distances between individuals over time, in body lengths."

video_fps = 29.97

fig, ax = plt.subplots()

# Initialise the image with the first frame, with a fixed colour scale
im = ax.imshow(
    dist_matrix.isel(time=0),
    cmap="viridis",
    vmin=0,
    vmax=dist_matrix.max().item()
)
ax.set_xlabel("individual j")
ax.set_ylabel("individual i")
ax.set_facecolor("black") # NaN values shown in black rather than white
title = ax.set_title("")

cbar = plt.colorbar(im)
cbar.set_label("distance (BL)")


# Define update function for animation
def update(t_i):
    """Update the image data with the distance matrix at frame ``t_i``."""
    im.set_data(dist_matrix.isel(time=t_i))
    title.set_text(f"t = {dist_matrix.time.values[t_i]:.2f} s")
    return im, title


# Create animation
frame_step = 60  # subsample frames to show data every 2 s approx.
frame_idcs = range(0, centroid.time.size, frame_step)
interval_from_fps_in_ms = (1 / video_fps) * 1000  # realtime interval between frames (ms)
anim = FuncAnimation(
    fig,
    update,
    frames=frame_idcs,
    interval=interval_from_fps_in_ms * 3,  # delay between frames in ms for replay
    blit=False,  # required because the title changes per frame
)

plt.close(fig)  # prevent the static first frame from being rendered too

HTML(anim.to_jshtml())
```

Watching the animation, we can see how the overall brightness of the matrix changes as the herd contracts and expands, and how blocks of individuals that stay close to each other appear as darker patches. Comparing these changes with the polarization and speed traces above is a good starting point for exploring how the herd reorganises around each escape event.
