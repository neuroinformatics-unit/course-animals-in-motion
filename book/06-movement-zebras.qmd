...
It would be useful to check if there are missing values in the body length array. We can quickly inspect this using `movement`'s [`report_nan_values()`](https://movement.neuroinformatics.dev/latest/api/movement.utils.reports.report_nan_values.html) function.
...
For this, we first compute each animal's **unit body vector**. These are a scaled version of the body vectors we just computed, normalised to have unit length. `movement` provides a convenience function to do this, [`convert_to_unit()`](https://movement.neuroinformatics.dev/latest/api/movement.utils.vector.convert_to_unit.html):
...
Polarisation over time alongside the unit body vectors, the herd vector and the drone footage.
:::
...
First, let's scale the filtered position data to express it in units of body lengths (BL). This will make the results more interpretable. We can use `movement`'s [`scale()`](https://movement.neuroinformatics.dev/latest/api/movement.transforms.scale.html) function to do this.
