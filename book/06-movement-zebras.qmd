...
Note that the scaling factor is the median body length computed over the _unfiltered_ data. The median is quite robust to outliers, so we leave it as is since recomputing it after filtering would make little difference.
...
The plot suggests that the individuals are quite coordinated also in their change of speed. We can clearly see the four periods of higher speed, which correspond to the four simulated escape events. The white gaps correspond to the samples we discarded, either because the individual was not detected or because its body length fell outside the range we defined above.
