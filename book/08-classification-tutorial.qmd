...
Only ~21% of frames are mounts, so a lazy classifier that *always* predicts `non-mount`
...
Let's compute one such feature with `movement`; we call it the **intruder-facing-resident angle**.
...
Then, we compute the intruder's forward-facing direction, using [`compute_forward_vector_angle()`](https://movement.neuroinformatics.dev/latest/api/movement.kinematics.compute_forward_vector_angle.html) with respect to that intruder-to-resident vector.
...
You can think about it in this way: in order to uniquely determine which way is forward for an animal, we need to know the orientation of the other two body axes: left-right and up-down. The left-right axis is specified by the left and right keypoints passed to the function, while we use the `camera_view` parameter to determine the upward direction (see image). The default view is "top_down", but it can also be "bottom-up".
...
Intuitively, the **forest** aggregates votes from multiple trees to make a final prediction.
...
Random forests are an excellent starting point: they are robust, require little tuning, and can handle features on different scales (pixels, radians, pixels/second) without the need for normalisation. Moreover, they don't require many computational resources, making them suitable for quick experimentation.
...
trained on. How well does it perform on a **completely new video**, with different animals, that it has never seen?
...
between videos (**domain shift**) may make performance drop on other videos.
You need to test on many videos that represent the full diversity of your dataset before you can trust the number.

## From frame predictions to behaviour bouts

Our classifier decides each frame **independently**, so its predictions flicker:
a single mount gets split into several fragments by a few stray `non-mount` frames, and vice-versa.
Real behaviour doesn't work like that—it comes in continuous **bouts**, just like the ones we annotated in BORIS (@sec-boris).

We can see how fragmented the raw predictions are by counting contiguous runs of `mount`
frames, and comparing that to the number of ground-truth bouts.
...
That's far more predicted bouts than there really are.
A simple fix is to **smooth the predictions in time**:
decide each frame's label by a majority vote over a short window around it,
so isolated flips get overruled by their neighbours.

We can do this with the very same [`rolling_filter`](https://movement.neuroinformatics.dev/latest/api/movement.filtering.rolling_filter.html)
we used to smooth the pose tracks at the start—only now we apply it to the predictions.
The filter works on time-aware `movement` arrays, so we first wrap `y_new_pred` into a
`DataArray` on the same time axis. The median of a window of 0s and 1s is essentially a
**majority vote**: it comes out on the `mount` side only when most frames in the window are mounts.

The filter treats our booleans as 0s and 1s and always returns **floats** (decimal numbers),
so we `round()` to the nearest whole number and convert back to `bool` to get a clean boolean mask.
...
Much closer to the true bout count; smoothing appears to have removed most of the flicker.
The clearest way to see the effect is to plot the three timelines on top of each other.
...
::: {.callout-note title="Choosing the window is a trade-off" collapse="true"}
...
Let's implement this as a function, `extract_bouts()`, that takes a boolean mask and returns a tidy table of bouts.
...
- **Richer kinematic features**: compute many diverse features, closer to how off-the-shelf superised classifiers operate. For example, see *Table 3* in @mars2021 and *Table S4* in @choudhary_jax_2025. Better to cast our net wide and let the model learn which features are useful, rather than hand-picking a few.
- **Off-the-shelf tools** built for similar supervised classification workflows basd on motion tracks and kinematic features (but some may be species-specific, or require a certain set of keypoints/skeleton):
...
- **Use more than just the current frame**: See *Figure 5* in @sun_caltech_2021. Sequence models with different architectures like RNNs and Transformers that can learn from the temporal structure of behaviour.
- **Features direct from video**: use tools like [FERAL](https://www.getferal.ai) [@skovorodnikov_feral_2026] and [DeepEthogram](https://github.com/jbohnslav/deepethogram) [@bohnslav_deepethogram_2021] for direct video-to-behaviour classification, without the intermediate step of pose estimation.
