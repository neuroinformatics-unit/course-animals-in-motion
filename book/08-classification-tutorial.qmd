...
How does the treen learn from the data? At each node, it tries every feature and threshold, and keeps the split that leaves the two
...
Our model classifies each frame **independently**, so its predictions flicker:
a single `mount` event gets split into several fragments by a few stray `non-mount` frames, and vice-versa. However, we expect this behaviour to appear in continuous **bouts**, just like the ones we annotated in BORIS (@sec-boris).
...
decide each frame's label by a majority vote over a short sliding window around it,
...
The statistic we would want to use is the **median**: over a window of 0s and 1s, it comes out on the
`mount` side only when most frames in the window are mounts, which is exactly a majority vote.

The filter works on time-aware `movement` arrays, so we first wrap `y_new_pred` as a
`DataArray` along the same time axis.
