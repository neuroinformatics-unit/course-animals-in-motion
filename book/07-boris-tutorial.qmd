...
## Supervised behaviour classification {#sec-supervised-classification}
...
Supervised classification is not the only way to segment behaviour. Other approaches include:

- **Manual annotation**: a human observer watches videos and annotates frames with behaviours of interest. You will get a taste of this in this chapter, but annotating every frame of every video in a large dataset is usually impractical.
...
- *Figure 3* in @pereira_quantifying_2020 for an alternative schematic representation of supervised and unsupervised approaches.
- *Table 1* in @goodwin_simple_2024 for an overview of open-source tools for behaviour segmentation, grouped by approach.

Whichever approach you choose, segmenting behaviour is hard and presents two kinds of **challenges**.

First, behaviour itself resists tidy labels:
...
Second, each family of methods comes with its own limitation:

- Rule-based methods rely on hand-picked thresholds that often fail to generalise across datasets.
...
## Choosing a behaviour segmentation approach

We have now glimpsed how manual annotation works in practice. The next chapter (@sec-classification) covers rule-based and supervised classification, building on the annotations we have just produced. Unsupervised methods are not covered in this current version of the handbook, but they remain a valuable part of the toolkit.

Whichever tools you end up using, the more important skill is knowing how to choose the right behaviour segmentation approach for the question at hand.

::: {.callout-tip title="Discuss"}
Which behaviour segmentation approach (see the *Approaches to behaviour segmentation* note in @sec-supervised-classification) would you choose in the following scenarios, and why?

1. You captured a single 5-minute video of a rare species in the wild and want to isolate a particular behaviour of interest.
2. You have a large dataset comparing two groups of mice and want to determine if they spend different amounts of time grooming.
3. You have videos tracking a few mice in their home cages over a period of weeks, and want to quantify how much time they spend locomoting vs being stationary.
4. You have created a new mutant fly line and want to characterise its behaviour repertoire, and compare it to wild-type flies.
5. You have hundreds of hours of video of primates in a zoo and want to find every occurrence of a specific, well-defined social interaction, even though it happens only rarely.
:::
