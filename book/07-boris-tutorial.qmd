...
In this tutorial, we will annotate a single, easily identifiable behaviour—the resident mounting the intruder—on the same video we used for the SLEAP tutorial: `mouse044_task1_annotator1.mp4`. These annotations can be used to train a **binary classifier**: at any given frame, the behaviour is either taking place or not.

::: {.callout-note}
The original "Task 1" defined by @sun_caltech_2021 is a **multi-class classification problem** aiming to identify four behavioural states—`attack`, `investigation`, `mount` and `other`. Here, we focus solely on `mount` in the interest of time, but the annotation workflow presented below could be easily extended by adding more behaviours to the ethogram.
:::
...
For a more complex worked example that makes extensive use of the "Exclusion matrix", see the [`movement` documentation: Annotate and load events with BORIS](https://movement.neuroinformatics.dev/dev/examples/advanced/annotate_boris_events.html).
:::
...
Click "Subject" → "Add subject" and add one subject named `resident_b`, with the "Key" `b` and an optional description.
