...
::: {.callout-note title="A side note on 2D vs 3D pose estimation" collapse="true"}

This tutorial focuses on **2D pose estimation**: locating each keypoint within the flat image plane of a single camera. This is the most common approach, and sufficient to answer many behavioural questions.
However, a camera image is only a projection of the real, 3D world onto a 2D sensor:
real-world distances and angles are distorted, and depth information is lost.

When behaviours inherently span three dimensions—such as reaching, climbing, rearing, or flight—we may need full **3D pose estimation**. This is achieved by combining videos from multiple synchronised camera views to reconstruct 3D coordinates. There are two main markerless approaches:
...
## Identity tracking approaches {#sec-identity-tracking-approaches}
...
## SLEAP workflow {#sec-workflow-sleap}
...
Add them all to the project upfront, then [sample frames](#sec-sample-frames) from across as many of them as possible when building your training set.
...
### Sample frames {#sec-sample-frames}
...
Here, we will employ the [appearance-based tracking approach](#sec-identity-tracking-approaches) for maintaining consistent identities of individual animals across frames.
...
### Monitor training progress {#sec-monitor-training-progress}
...
You can stop a training run at any time and resume it later, or start a new run using the weights from a previous model.
For details, see @sec-continue-training-across-sessions.
...
::: {.callout-note}
Since we have trained an [appearance-based ID model](#sec-identity-tracking-approaches), which outputs identity assignments directly for each detected animal in each frame, there is no need to configure a "Tracker" (typically used for temporal-based tracking).
...
### Continue training across sessions {#sec-continue-training-across-sessions}

SLEAP lets you stop a run early, pick it up again later, or start over from scratch.

#### Stop a run early

While a model is [training](#sec-monitor-training-progress), the training window offers two ways to end it early:
