...
![](img/SLEAP/part_grouping.png){fig-align="center" width=80%}
...
![](img/SLEAP/pose_estimation_topdown.png){width=100%}
...
![](img/SLEAP/pose_estimation_bottomup.png){width=100%}
...
To address the problem of maintaining animal identities across frames, SLEAP implements two main classes of techniques: temporal-based and appearance-based cues.

**Temporal-based tracking** uses optical flow to estimate the displacement of poses across frames, associating past poses with new ones.
This method does not require model training, meaning users can perform tracking without additional labelling of consecutive frames, making it suitable for visually hard-to-distinguish animals.
However, a significant drawback is that errors, such as identity switches, propagate over time, making it less suitable for very long videos or real-time applications that cannot be proofread.

**Appearance-based tracking (ID models)** relies purely on appearance-based features to simultaneously assign identities while detecting and grouping landmarks, thereby addressing the issue of error propagation from temporal models.
It does, however, require animals with sufficiently distinctive appearance cues so that they can be manually identified during labelling.

- **Top-down ID models** modify the centred-instance network to additionally predict class probabilities for each animal-centred crop.
- **Bottom-up ID models** replace PAFs with multi-class segmentation maps, where masks for body parts are collapsed into distinct channels for each unique animal class ID. Grouping is implicit in the ID assignment.
...
We will also assign identities to each instance by adding a `Track`.
To do this, select an instance and click on "Tracks" then "Set Instance Track" in the top menu bar.
