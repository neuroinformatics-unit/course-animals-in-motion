...
Before we proceed, make sure you have installed [SLEAP](http://legacy.sleap.ai/installation.html) [@pereira_sleap_2022] and activated the corresponding `conda` environment (see [prerequisites @sec-install-sleap]).
...
::: {.callout-warning}
This workshop uses **SLEAP version 1.3.4** with a TensorFlow backend.
Documentation for this version is available at [legacy.sleap.ai](http://legacy.sleap.ai/).

From **SLEAP 1.5.0 onwards**, the software uses a PyTorch backend.
Installation and usage instructions differ for these newer versions, with updated docs at [docs.sleap.ai](https://docs.sleap.ai/).
:::
...
Further details can be found in [SLEAP's Creating a project guide](http://legacy.sleap.ai/tutorials/new-project.html).
...
For assembling a set of frames for annotation, you can either pick your own frames, or let SLEAP suggest a set of frames using the "Labeling Suggestions" panel, which offers several [automated sampling strategies](http://legacy.sleap.ai/guides/gui.html#suggestion-methods) to help select informative and diverse frames.
...
See also [SLEAP's GUI guide on "Labels"](http://legacy.sleap.ai/guides/gui.html#labels) for a complete reference of all labelling functions.
...
See also [SLEAP's Configuring models guide](http://legacy.sleap.ai/guides/choosing-models.html) for further details on model types and training options.
...
For detailed instructions and guidance, see the [Running SLEAP remotely guide](http://legacy.sleap.ai/guides/remote.html).
:::
