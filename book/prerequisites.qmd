...
You will also need a separate `conda` environment for the tutorials and case studies involving [movement](https://movement.neuroinformatics.dev/). This environment includes `movement` itself as well as [jupyter](https://jupyter.org/), which is required for following along interactively and solving the exercises.
...
- **CalMS21**: Contains an example video from the [Caltech Mouse Social Interactions (CalMS21) Dataset](https://sites.google.com/view/computational-behavior/our-datasets/calms21-dataset) [@sun_caltech_2021], SLEAP labels and trained models, as well as BORIS-compatible annotations of "mount" events.
  ...
  - **mouse044_task1_annotator1.mp4**: Video file used in [@sec-sleap] and [@sec-boris].
  ...
  - **mouse044_task1_mount_events_boris.tsv**: Ground-truth annotations of "mount" events in the video, extracted from `calms21_task1_train.json` to a BORIS-compatible TSV format. Used in [@sec-boris].
