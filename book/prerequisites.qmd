...
- [brew](https://brew.sh/) (for *macOS users* only)
...
### For movement tutorials {#sec-install-movement}

You will also need a separate `conda` environment for the tutorials and case studies involving [movement](https://movement.neuroinformatics.dev/). This environment includes `movement` itself as well as [jupyter](https://jupyter.org/) (required for following along interactively and solving the exercises) and [scikit-learn](https://scikit-learn.org/stable/) (required for @sec-classification).
...
### For the BORIS tutorial {#sec-install-boris}

We recommend following the official [BORIS installation instructions](https://www.boris.unito.it) for your operating system.

::: {.callout-note}
For *macOS users*, BORIS installation requires `brew` and `uv`, which are both listed under [general development tools](#sec-install-devtools). You may safely ignore the warning about this being an *experimental* version of BORIS.
:::
...
Please download these from [Dropbox](https://www.dropbox.com/scl/fo/81ug5hoy9msc7v7bteqa0/AH32RLdbZqWZJstIeR4YHZY?rlkey=blgagtaizw8aac5areja6h7q1&st=xni448zl&dl=0) before the workshop starts (they are about 11 GB on disk once expanded).
...
│   ├── mouse026_task1_annotator1.slp
│   ├── mouse026_task1_mount_events_boris.tsv
...
- **CalMS21**: Contains an example video from the [Caltech Mouse Social Interactions (CalMS21) Dataset](https://sites.google.com/view/computational-behavior/our-datasets/calms21-dataset) [@sun_caltech_2021], SLEAP labels and trained models, as well as BORIS-compatible annotations of "mount" events.
  ...
  - **mouse026_task1_annotator1.slp**: Fully annotated SLEAP labels file (converted from `calms21_task1_train.json`).
  - **mouse026_task1_mount_events_boris.tsv**: Ground-truth annotations of "mount" events, extracted from `calms21_task1_train.json` to a BORIS-compatible TSV format.
  - **mouse044_task1_annotator1.mp4**: Video file.
  - **mouse044_task1_annotator1.slp**: Fully annotated SLEAP labels file (converted from `calms21_task1_train.json`). The train/test splits above were derived from this file using a 20:80 random partition.
  - **mouse044_task1_mount_events_boris.tsv**: Ground-truth annotations of "mount" events, extracted from `calms21_task1_train.json` to a BORIS-compatible TSV format.
