...
To prepare your computer, please install the following:

- [Git](https://carpentries.github.io/workshop-template/install_instructions/#git) and create a [GitHub account](https://github.com/signup) if you don't already have one
...
Create a conda environment with Python 3.13 and activate it:

```bash
conda create -n sleap_env python=3.13
...
```bash
# CPU only
pip install "sleap[nn]==1.6.3" --extra-index-url https://download.pytorch.org/whl/cpu

# NVIDIA GPU (CUDA 12.8)
pip install "sleap[nn]==1.6.3" --extra-index-url https://download.pytorch.org/whl/cu128
```
...
::: {.callout-note}
The installation command installs SLEAP v1.6.3. To install a different version, replace `1.6.3` with the desired version number in the command above.
:::
