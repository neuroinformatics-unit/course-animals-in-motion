...
some interventions during that time, which are out of scope for this analysis.
...
by the mouse's body centroid in a given time bin—in this case, 15 minutes.
Since we have already computed the speed (cm/sec), we can convert that to distance (cm) by multiplying by the time bin duration in seconds.
...
lights_off = pd.Timedelta(hours=9, minutes=30).total_seconds() / 60
dawn = pd.Timedelta(hours=20, minutes=30).total_seconds() / 60
lights_on = pd.Timedelta(hours=21, minutes=30).total_seconds() / 60

n_bins_in_day = int(24 * 60 / time_bin_minutes)

max_activity = max(
    activity_dict[kage].max().values for kage in ["kage14", "kage17"]
)
...
for i, kage in enumerate(["kage14", "kage17"]):
    ...
    # Create an empty actogram with dims (date, time_bin)
    actogram = xr.DataArray(
        np.zeros((len(days), n_bins_in_day)),
        dims=["date", "time_of_day"],
        coords={
            "date": days,
            # Each time bin has a time coordinate corresponding to
            # the number of minutes between midnight and the bin centre.
            "time_of_day": np.arange(
                time_bin_minutes/2, 24 * 60, time_bin_minutes
            )
        },
    )
    # Populate actogram per day
...
::: {.callout-tip title="Exercise D"}

1. How would you describe the observed differences between the two mice?
2. Please quantify the mean +/- standard deviation of the activity levels during the light and dark periods, for each mouse.

_Hint_: Start with the `actogram_dict` dictionary defined within the plot code.

:::

::: {.content-visible when-profile="answers"}

### Solutions: Exercise D
...
We can quantify the mean and standard deviation of the activity levels during
the light and dark periods, as follows:

Let's examine the structure of the actogram data.

```{python}
actogram_dict["kage14"]
```

```{python}
for kage, actogram in actogram_dict.items():
    # Dark period is from lights off to lights on
    dark_period = slice(lights_off, lights_on)
    dark_data = actogram.sel(time_of_day=dark_period)
    dark_mean = dark_data.mean().values
    dark_std = dark_data.std().values

    # Light period is from dawn to lights on
    # Light period consists of two parts: before lights off and after lights on
    light_period1 = slice(None, lights_off)
    light_period2 = slice(lights_on, None)
    light_data = xr.concat(
        [
            actogram.sel(time_of_day=light_period1),
            actogram.sel(time_of_day=light_period2)
        ],
        dim="time_of_day"
    )
    light_mean = light_data.mean().values
    light_std = light_data.std().values
    print(
        f"{kage} activity in light period: "
        f"{light_mean:.2f} ± {light_std:.2f}"
    )
    print(
        f"{kage} activity in dark period: "
        f"{dark_mean:.2f} ± {dark_std:.2f}\n"
    )
```
