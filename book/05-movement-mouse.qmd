...
netCDF is an HDF5-based file format that can be natively saved/loaded by the `xarray` library, and is therefore [convenient to use with `movement`](https://movement.neuroinformatics.dev/latest/user_guide/input_output.html#native-saving-and-loading-with-netcdf).
...
for i, kage in enumerate(["kage14", "kage17"]):
    ds = ds_kages[kage].squeeze()  # remove redundant "individual" dimension
...
for i, kage in enumerate(["kage14", "kage17"]):
    ...
    ds_daily.confidence.sel(keypoint="bodycenter").plot.line(ax=axes[i])
...
for kage, ds in ds_kages.items():
    ds["body_centroid"] = ds.position_filtered.sel(
        individual="individual_0",  # the only individual in the dataset
        keypoint=reliable_keypoints
    ).mean(dim="keypoint")
```

Next, we'll compute the body centroid's speed in cm/sec via the following steps:

1. Convert the body centroid position data to cm units using [`scale()`](https://movement.neuroinformatics.dev/latest/api/movement.transforms.scale.html).
2. Temporarily switch to "elapsed time" mode, because [`compute_speed()`](https://movement.neuroinformatics.dev/latest/api/movement.kinematics.compute_speed.html) does not (yet) support datetime coordinates.
...
`movement` provides a [`plot_occupancy()`](https://movement.neuroinformatics.dev/latest/api/movement.plots.plot_occupancy.html) function to help us visualise the space occupancy.
...
Here we'll use `movement`'s [functionality for defining regions of interest (ROIs)](https://movement.neuroinformatics.dev/latest/api/movement.roi.html).
...
[built-in methods](https://movement.neuroinformatics.dev/latest/api/movement.roi.PolygonOfInterest.html).
