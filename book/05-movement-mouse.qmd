...
::: {.callout-tip title="`mean` instead of `sum` and divide"}
...
The time spent in a region is an aggregate metric across the entire session. To understand how this dwell time accumulated over the course of the session, the number of entries into each region offers a useful complementary measure—one that can be extracted almost trivially from the boolean `occupancy` array.
...
This simple countis subject to a few spurious effects.
