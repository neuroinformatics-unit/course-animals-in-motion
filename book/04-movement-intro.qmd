...
Each frame-level prediction has a small error, and when these are connected into tracks, the resulting trajectories appear jittery.
...
This function applies a rolling window across the `time` dimension
of the input data array (typically `position`) and compute a specified statistic
over each window. *How is that useful?*
