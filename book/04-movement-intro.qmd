...
Referring back to @fig-plot-tail-base-pos, we can see that the position trajectories exhibit some errors:
...
As a starting point for all quantifications, we will use ground-truth position
data that you have downloaded from the CalMS21 dataset, `mouse044_task1_annotator1.slp`.

```{python}
ds_gt = load_dataset(
    Path.home() / ".movement" / "CalMS21" / "mouse044_task1_annotator1.slp",
    fps=30,
)
position = ds_gt.position

position
```

Our dataset tracks 7 keypoints per mouse, but many questions are easier to ask
of a single point per animal. So, as we did in the previous section, let's reduce
each mouse to its centroid.

```{python}
centroid_pos = position.mean(dim="keypoint")
```
...
We can answer this with the
[`compute_pairwise_distances`](https://movement.neuroinformatics.dev/latest/api/movement.kinematics.compute_pairwise_distances.html)
function, which computes distances between pairs of individuals—or, if we prefer,
...
distance.plot.line(x="time", aspect=2, size=2.5)
```

The two mice repeatedly come together and move apart again, so this single variable
already gives us a compact summary of the encounter.

::: {.callout-tip title="Discuss"}
- Roughly how much of the session do the mice spend near each other?
- The distance is measured in pixels. What would you need to know about the recording
  setup to express it in centimetres instead?
- We computed the distance between centroids. What would change if we used the distance
  between the two mice's noses instead? Which would you choose, and why?
:::

::: {.exercise-prompt}
Knowing *where* the mice are relative to each other tells us when they meet, but not
what they do when they get there. Speed adds that missing piece.
...
2. Plot the speed of both mice on the same axes, with one colour per individual.
   Which mouse is more active overall?
3. Now plot the speed and the inter-mouse distance one above the other, sharing a
   common time axis. What relationships—if any—can you spot between the two?

__Bonus__

4. The raw speed traces are very spiky, which makes them hard to read. Try smoothing
   them with a rolling median (e.g. `speed.rolling(time=15, center=True).median()`)
   before plotting. Does the picture become clearer? What might you be losing?
:::
...
speed = compute_speed(centroid_pos)
speed
```

```{python}
speed.plot.line(x="time", hue="individual", aspect=2, size=2.5)
```

To compare the two variables, we place them in two stacked subplots that share the
same time axis. Here we use `matplotlib` directly to set up the figure, and then tell
each `plot.line` call which subplot (`ax`) to draw into.

```{python}
#| label: fig-plot-speed-and-distance
#| fig-cap: "Inter-mouse distance (top) and the speed of each mouse (bottom)."

import matplotlib.pyplot as plt

fig, axes = plt.subplots(2, 1, sharex=True, figsize=(8, 5))

distance.plot.line(x="time", ax=axes[0], color="black")
axes[0].set_ylabel("distance (pixels)")
axes[0].set_xlabel("")

speed.plot.line(x="time", hue="individual", ax=axes[1])
axes[1].set_ylabel("speed (pixels/s)")
...
A rolling median replaces each value with the median of the 15 frames (0.5 s) centred
on it, which suppresses the frame-to-frame spikes while preserving the slower changes.

```{python}
speed_smooth = speed.rolling(time=15, center=True).median()
...
speed_smooth.plot.line(x="time", hue="individual", ax=axes[1])
axes[1].set_ylabel("speed (pixels/s)")

fig.tight_layout()
```

The traces are now much easier to follow, but we have traded away temporal precision:
brief, genuine bursts of movement are flattened along with the noise. The wider the
smoothing window, the more of the real signal we sacrifice.
:::
